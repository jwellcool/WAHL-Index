 ...
 public:
  ...
  void Build(const std::vector<std::pair<uint64_t, uint64_t>>& data) {
    ...
    for (uint64_t i = 0; i < data.size(); ++i) {
        ...
        Insert(data_key, data_value);
    }
  }

    void Insert(uint64_t data_key, uintptr_t data_value) {
        uint8_t key[8];
        swapBytes(data_key, key);
        insert(tree_, &tree_, key, 0, data_value, 8);
    }

    void* Lookup(uint64_t lookup_key)  {
        uint8_t reverse_key[8];
        swapBytes(lookup_key, reverse_key);
        LeafNode* leaf = lookup(tree_,reverse_key, 8, 0,8);

        if (leaf) {
            return reinterpret_cast<void*>(leaf->value);
        }
        return nullptr;
    }

    void* LowerBound(uint64_t lookup_key) {
        uint8_t reverse_key[8];
        swapBytes(lookup_key, reverse_key);
        // Lower bound lookup.
        Iterator it;
        const bool found = bound(tree_, reverse_key, 8, it, 8, true);
        if (!found) return nullptr;
        return (void*)it.value;
    }


    void Range(uint64_t start_key, uint64_t end_key, std::vector<std::pair<uint64_t, uint64_t>> &data) {
        uint8_t reverse_key[8];
        ...
        uint64_t key;
        while (true) {
            LeafNode *leaf = (LeafNode *)it.value;
...
// This address is used to communicate that search failed
  ...
  bool leafMatches(Node* leaf,
    ...
    if (depth!=keyLength) {
      uint8_t *leafKey = loadKey((LeafNode*)getLeafValue(leaf));
    ...
    if (node->prefixLength > maxPrefixLength) {
      for (pos = 0; pos < maxPrefixLength; pos++)
        if (key[depth + pos]!=node->prefix[pos])
          return pos;
      uint8_t *minKey = loadKey((LeafNode*)getLeafValue(minimum(node)));
...
// Compare the the key with the prefix, return the number matching bytes
  {
    ...
    if (compBytes > maxPrefixLength) {
      ...
      Node* minNode = minimum(node);
        uint8_t *minKey = loadKey((LeafNode*)getLeafValue(minNode));
  ...
  Node* lookupPrefix(Node* node,
    ...
    if (isLeaf(node)) {
      ...
      uint8_t* existingKey = loadKey((LeafNode*)getLeafValue(node));
      // Check if it matches
  ...
  bool bound(Node* n,
    ...
    while (true) {
      ...
      if (isLeaf(n)) {
        ...
        uint8_t* leafKey = loadKey((LeafNode*)getLeafValue(n));
        for (unsigned i = depth; i < keyLength; i++)
          if (leafKey[i]!=key[i]) {
            if (leafKey[i] < key[i]) {
              // Less
    ...
    while (node!=NULL) {
      if (isLeaf(node)) {
        if (!skippedPrefix && depth==keyLength) // No check required
          return (LeafNode*)getLeafValue(node);

        if (depth!=keyLength) {
          // Check leaf
          uint8_t* leafKey =  loadKey((LeafNode*)getLeafValue(node));
...
//void insertNode256(Node256* node, Node** nodeRef, uint8_t keyByte, Node* child);
  ...
  void insert(Node* node,
    ...
    Node* newNode = makeLeaf(key, value);
    switch (node->type) {
      case NodeType4:
...
uint64_t ARTPrimaryLB::allocated_byte_count;
uint64_t ARTPrimaryLB::node4_num;
uint64_t ARTPrimaryLB::node16_num;
uint64_t ARTPrimaryLB::node48_num;
uint64_t ARTPrimaryLB::node256_num;
